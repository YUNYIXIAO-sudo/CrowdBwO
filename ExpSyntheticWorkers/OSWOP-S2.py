...
for i in range(para.workerGroupsNumber):    
    workerset = i
    f = open('./expAVEResults/OSWOP-S2averageResultsWS' + str(i) + '.csv', mode='w')
    index = ''
    ...
    for j in range(para.expNumber):
        ...
        df.to_csv('./expResults/OSWOP-S2resultsW' + str(workerset) + 'E' + str(experiment) + '.csv', mode='w')
        ...
        df.to_csv('./expAVEResults/OSWOP-S2averageResultsWS' + str(i) + '.csv', mode='a', index=False, header=False)
