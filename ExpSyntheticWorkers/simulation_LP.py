...
class simulation(object):
    ...
    def firstTrial(self):
        ...
        while fee < (B * beta) and self.t < timeMax: 
        ...
        for n in range(N): #number of simulations
            ...
            while pullingIsFeasible == 1:
                ...
                if self.m == 0:
                    ...
                    if self.t == self.tau and self.t < T - np.array(para.batchCreateTime[workerset], dtype=np.int32)[0]:
                        ...
                        if self.m % updateFrequency == 1:
                            ...
                            #print(self.thetas, len(self.timeStamp), 'trained para'
                            countModelTrain += 1
                        ...
                        estimatedTraffic, estimatedFee, cutTime = estimateTraffic.getTraffic(T - self.t, self.thetas, self.thetas_std, experiment, workerset)
                        '''if workerset == 1:
                            print('m:', self.m, 'cutTime:', cutTime, 'timemodel:', self.thetas)'''
                       
                       
                        estimatedDensityTraffic, estimatedDensityFee, timeDensity = estimateTraffic.getDensityTraffic(T - self.t, self.thetas, self.thetas_std, experiment, workerset)
                        

                        pullingArms, cuttingMethod = agent.get_arms((T - self.t), (B - self.usedBudget - addFeeforRecords), estimatedTraffic, estimatedFee, estimatedDensityTraffic, estimatedDensityFee)
                        ...
                        '''pullIndex = np.where(pullingArmsThisTime == 1)
                        goOnPulling = np.intersect1d(sameArms, pullIndex)'''

                        #print(sameArms, self.pullingArms, pullingArmsThisTime, plusTime, time, 'check same arm')
                        ...
                        self.tau += batchTime
                        #print(self.rewards, self.pulledCounts, self.m, self.t, self.usedBudget, time, 'result of a batch')
                    ...
                    #print(timeStamp, self.batchTime, plusTime, self.usedBudget, countWorkersThisTime, 'check time stamp')
                    ...
                    #print(self.cutTime, self.countCutTime, timeStamp, 'checkCutTimeR')
...
for i in range(para.workerGroupsNumber):    
    ...
    for j in range(para.expNumber):
        ...
        if __name__ == "__main__":
            ...
            df.to_csv('./expResults/CrowdBwO-S1resultsW' + str(workerset) + 'E' + str(experiment) + '.csv', mode='w')
            ...
            df.to_csv('./expAVEResults/CrowdBwO-S1averageResultsWS' + str(i) + '.csv', mode='a', index=False, header=False)
