...
for n in range(para.workerGroupsNumber):
    ...
    for alg in algs:
        ...
        print(globals()[alg + 'selectedArms'], globals()[alg + 'time'])
        ...
        for a in range(para.nArms[n]):
            plt.bar(para.budget[:para.expNumber] - ((width * para.nArms[n]) / 2) + width * (a + 0.5), pulledTimePercentages[a], width, color=barColors[a], edgecolor='#5d5e5e', label=barLabels[a])
    ...
    plt.figtext(0.004, 0.16, 'Time Allocation(%)', fontsize=30, rotation='vertical')
