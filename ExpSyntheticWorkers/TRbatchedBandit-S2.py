...
class simulation(object):
    ...
    def sim(self, Agent):
        ...
        T = para.totalRoundsBBS2[workerset][experiment]
        ...
        for n in range(N): #number of simulations
            ...
            while pullingIsFeasible == 1:
                ...
                if self.t == self.tau:
                    ...
                    self.m += 1
                    if self.m > M:
                        print(experiment, np.sum(self.rewards), self.t, 'finalcheck')
                        self.recordsPrint()
                        break
                ...
                if np.sum(self.rewards + reward) >= para.answersRequired[experiment] or self.t >= para.S2timeLimit[experiment]:
                    print(experiment, np.sum(self.rewards), self.t, 'finalcheck')
                    self.pulledCounts += workersThisTime
                    self.rewards += reward
                    fee = feeCount(workersThisTime, experiment, workerset)
...
for i in range(para.workerGroupsNumber):    
    ...
    for j in range(para.expNumber):
        ...
        #print('final results:', np.sum(np.mean(np.array(selectedArmsR), axis=0)), averageResults)
