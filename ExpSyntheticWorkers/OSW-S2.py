...
class OSW(object):
    ...
    def S2(self):
        ...
        t = 0
        while np.any(AccumulatedRewards < RequiredResults) == 1 and t < para.S2timeLimit[self.exp]:
            #print('a')
            t += 1
            workers = estimateTraffic.predict(t, self.thetas, np.zeros(shape=(4, para.nArms[workerset])), self.exp, self.workerset)
            AccumulatedRewards = (workers * np.array(para.rewardProbs[workerset], dtype=np.float64)).copy()
            fees[AccumulatedRewards <= RequiredResults] = feeCountSeperate(workers, self.exp, self.workerset)[AccumulatedRewards <= RequiredResults]
...
for i in range(para.workerGroupsNumber):    
    workerset = i
    f = open('./expAVEResults/OSW-S2averageResultsWS' + str(i) + '.csv', mode='w')
    index = ''
    ...
    for j in range(para.expNumber):
        ...
        si = simulation()
        selectedArmsR, rewardsR, pulledR, budgetR, tR = si.sim(OSW)
        ...
        df.to_csv('./expResults/OSW-S2resultsW' + str(workerset) + 'E' + str(experiment) + '.csv', mode='w')
        ...
        df.to_csv('./expAVEResults/OSW-S2averageResultsWS' + str(i) + '.csv', mode='a', index=False, header=False)
