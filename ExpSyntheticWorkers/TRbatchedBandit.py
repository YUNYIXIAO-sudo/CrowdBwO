...
class simulation(object):
    ...
    def sim(self, Agent):
        ...
        for n in range(N): #number of simulations
            ...
            M = int(np.log2(np.log2(T)))
            alpha = int(T ** (1 / (2 - (2 ** (1 - M)))))
            ...
            while pullingIsFeasible == 1:
                ...
                if self.t == self.tau:
                    if self.m == 0:
                        ...
                        self.pulledCounts += workersThisTime

                        reward = env.reactRewards(pullingArmsThisTime, workersThisTime, experiment, workerset)
                        ...
                        fee = feeCount(workersThisTime, experiment, workerset)
                ...
                if self.t < self.tau - left:
                    
                    batchTime += 1
                    
                    realWorkers = env.reactWorkerNum(pullingArmsThisTime, batchTime, 0, experiment, workerset)
        
                    workersThisTime = realWorkers.copy()
                    
                    self.pulledTime += pullingArmsThisTime

                    self.t += np.sum(pullingArmsThisTime)
                    


                elif self.t >= self.tau - left:
                    ...
                    workersThisTime[indexofPullingArms] = realWorkers[indexofPullingArms]
                ...
                if self.t == self.tau - left:
                    ...
                    self.pulledCountsforEstimate += workersThisTime
                ...
                #print(self.m, self.t, self.usedBudget, self.pulledCounts, self.rewards, pullingArmsThisTime)
                
                fee = feeCount(workersThisTime, experiment, workerset)
                
                if self.t >= T or self.usedBudget + fee >= B:
                    self.pulledCounts += workersThisTime
                    reward = env.reactRewards(pullingArmsThisTime, workersThisTime, experiment, workerset)
                    self.rewards += reward
                    self.usedBudget += fee
...
for i in range(para.workerGroupsNumber):    
    workerset = i
    f = open('./expAVEResults/BB-S1averageResultsWS' + str(i) + '.csv', mode='w')
    index = ''
    ...
    for j in range(para.expNumber):
        ...
        df.to_csv('./expResults/BB-S1resultsW' + str(workerset) + 'E' + str(experiment) + '.csv', mode='w')
        ...
        df.to_csv('./expAVEResults/BB-S1averageResultsWS' + str(i) + '.csv', mode='a', index=False, header=False)
