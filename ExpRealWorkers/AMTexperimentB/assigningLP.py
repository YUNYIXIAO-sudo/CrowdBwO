...
def analyzeResults(workers, rewards, time, qualification):
    collectedAnswers = workers + np.sqrt(np.array([30, 30]) * time).astype(np.int64)
    correctAnswers = rewards + np.sqrt(np.array([20, 20]) * time).astype(np.int64)
...
#mturk '3Z2GNG0CT9KNHVJOP25QD1WR4SIXWD'
...
class assigningProcess(object):
    ...
    def assign(self, Agent):
        ...
        return self.selectedArmsR, self.rewardsR, self.pulledCountsR, self.BudgetR, self.tR, self.meanR, self.stdR, self.updateFrequencyR, self.realTimeR
