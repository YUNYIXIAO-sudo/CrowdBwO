...
trainWindow = para.trainWindow


class workerTraffic():
    def train(countTime, pulledRec):
        ...
        if len(countTime) >= trainWindow:
            countTime = countTime[-trainWindow:]
            pulledRec = pulledRec[-trainWindow:]
        ...
        with pm.Model() as model:
            # Priors
            sigma = pm.HalfNormal('sigma', sd = para.sd[0], shape=(nArms,))
            theta0 = pm.Normal('theta0', mu = para.mu[0], sd = para.sd[1], shape=(nArms,))
            theta1 = pm.Normal('theta1', mu = para.mu[1], sd = para.sd[2], shape=(nArms,))
            theta2 = pm.Normal('theta2', mu = para.mu[2], sd = para.sd[3], shape=(nArms,))
            theta3 = pm.Normal('theta3', mu = para.mu[3], sd = para.sd[4], shape=(nArms,))
