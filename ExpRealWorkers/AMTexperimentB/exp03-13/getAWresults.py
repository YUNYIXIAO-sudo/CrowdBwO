...
results = pd.read_csv('./exp03-13/AssignmentResults13.csv')
...
for i in range(len(time)):
    ...
    splits = splits.split(':')
    time[i] = (int(splits[0]) - 20) * 60 + int(splits[1]) - 50
...
for i in range(1, time[-1]+1):
    ...
    if usedBudget1 + usedBudget2 >= 1500:
        PullingisFeasible *= 0
...
df.to_csv('./exp03-13/realEXPresultsAWLP13.csv', mode='w')